...
# Supporting Non-Fungible Token (NFT) or ERC721 Tokens on Open-Attestation Platforms with Minimal Code
...
1. Prescribe an interface that NFT contracts have to inherit that will make them act like `documentStore`
2. Create a code branch in OA platform to use `tokenStore` to reflect that this "document" is not just a document but also a transferable token
...
Provide a reference token contract conforming to ERC721 and is "compatible" with the OA Platforms. This contract can exist as reference code or be deployed via factories. 
...
### Display Ownership Information in Viewer
Viewing the transferable records in the viewer, the user should be able to identify the current owner of the asset in the form of a pseudonym, in this case ethereum address. 

Eg.
Token Owner: 0x12..34

In the implementation we may want to implement [identicons](https://en.wikipedia.org/wiki/Identicon) to allow users to quickly compare the values across different medium.

### Display Ownership Type
The user should be notified if the owner is a smart contract, and possibly link to the smart contract on Etherscan. This allow a different interaction flow (ie. through another Dapp) if another smart contract is involved. 

Eg.
Token Owner: 0x12..34 (smart contract)

### Differentiate Owner from Non-owner
If the user is the owner of the token, the viewer will reflect that. Later, interactions for owners can be enabled. Such interaction includes:

- Sign ownership request
- Transfer token

If the user is not the owner, he will have the below interactions:

- Generate ownership request
- Verify ownership request

Eg. 
Token Owner: 0x12..24 (YOU)

### Request for Ownership Verification
Allow user to generate a ownership verification message.

Implementation:
Sign `entropy`

### Sign Ownership Verification Request
Allow owner of a token to sign ownership message.

Implementation:
Check verification request is not a transaction
Sign challenge with private key

### Verify Ownership
Verify signed request against initial challenge

### NFT Token Factory
Smart contract to deploy a standard NFT Token Contract for shipping lines who are creating these transferable records.

### Out-of-band Communication Standards/Implementation
- Challenge-Response for ownership verification
- Token transfer notification
- Document data transfer notification

### Token Creator Dapp (aka admin panel)
Given document data => Generate "batched" document => Mint token on token contract

## Appendix

### Solidity Code for ERC721
